...
[Voice is enabled in Copilot Studio](/microsoft-copilot-studio/voice-get-started) or use the voice IVR template to access the answer machine detection system topic.
...
1. Select the system topic, and add the node to play the desired message. The message can be personalized based on the context of proactive engagement passed to the IVR agent. If you need to, you can chose to end the call instead of playing the message
...
1. To have your Copilot agent read context variables from Dynamics 365 Contact Center, do the following steps:
...
At runtime the variable within your Copilot agent is set to the value from Dynamics 365 Contact Center, and can be used for consumption.
