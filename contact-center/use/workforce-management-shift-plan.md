...
As a customer service supervisor, you use shift plans to plan and schedule customer service representatives (service representatives or representatives) based on availability, skills, and business requirements. Shift plans help ensure your contact center has the appropriate coverage to support daily operations.
...
Your administrator must enable the feature in Copilot Service admin center. Learn more in [Enable shift and schedule management](../administer/workforce-management-enable-schedule-management.md).
...
1. On the **Plan Details** card, enter the following details:

   - **Start date**: The date on which the shift plan begins, based on the time zone defined for the plan.
   ...
   - **End time**: The time the shift plan ends, as defined by the weekly recurrence.

     > [!NOTE]
     > The time difference between start and end time must be 24 hours. To create an overnight shift, set the start time to 8 PM and the end time to 8 AM. The time selector displays **+1 day** to indicate that the shift ends the next day.

   - **Capacity plan**: The number of service representatives required during the shift.

     > [!NOTE]
     > Select a capacity plan to use the forecasted capacity for each time interval to optimize staffing.

   - **Shift Rotation Policy**: Select the applicable shift rotation policy.
   - **Planning Group**: Select the applicable planning group. The shift plan inherits the planning group's time zone.
...
1. On the **Activity Itinerary** card, select **Add activity**, and then select the activities you want to add the shift from the dropdown menu. Hover on the activity to update the duration of an activity after adding it to the **Activity Itinerary**. Handle bars appear on the activity box. You can select the respective handle bar, and then drag it to increase or decrease the duration of the activity. You can also change the order of activities or delete them as follows:
