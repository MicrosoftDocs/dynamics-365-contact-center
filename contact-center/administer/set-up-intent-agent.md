...
- If the issue is escalated, it's transferred to representatives. Customer Intent Agent persists the intent and the interview questions generated by the Copilot agents. The service representative goes through unanswered questions with customer and gets an updated solution.
...
Enrichment Context lets Copilot agents use information collected before the intent is identified, so customers don’t have to repeat details. For example, a pre-chat survey can send data like contact info or account numbers to Customer Intent Agent through the Global.EnrichmentContext parameter. This ensures key details are available at the beginning, helping with intent classification, interview flows, and fulfillment. By reducing repeated questions, Enrichment Context improves customer experience and speeds up issue resolution.
...
- For each new conversation, administrators can configure EnrichmentContext to pass pre-collected details into Customer Intent Agent. 
...
1. Make sure your Microsoft Copilot Studio agent has global variables for each survey question.
