...
description: Learn how customer service representatives can use Copilot to draft, refine, and send email responses.
author: gandhamm
ms.author: mgandham
...
1. In Copilot Service workspace, open the Copilot help pane.
...
[!INCLUDE [cc-write-email-prompts](../../shared/cc-write-email-prompts.md)]
  ...
  Copilot displays the email template used to generate the draft. You can select **Choose a different template** if the template isn’t appropriate. The following actions are applicable:
   ...
   - Multiturn responses and the **Use knowledge sources** toggle aren't available when Copilot generates an email draft by using a template.
  ...
  1. Place your cursor where you want to generate partial text in the email body.
  1. Select **Draft with Copilot**. The **Draft with Copilot** card appears.
  ...
  1. Turn on **Use knowledge sources** to use knowledge sources to generate the draft.
