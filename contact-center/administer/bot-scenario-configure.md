...
- **self-service/account lookup**: Enables callers to access account information or perform self-service tasks without AI agent assistance, improving efficiency and customer satisfaction.
...
1. In the Agents page, you see the voice agent. You can modify the AI agent's workflow setting, topics, and call routing to align with your customer service processes and scenarios.
1. To finish the AI agent configuration in Dynamics 365 Contact Center or Customer Service, perform the steps in [Set up voice agents in the voice channel](/dynamics365/customer-service/administer/voice-channel-pva-bots?context=/dynamics365/contact-center/context/administer-context).
 ...
 > [!NOTE]
 > We recommend that you add the AI agent to the voice workstream in the omnichannel app and test it before you make changes to the topics.

## Contoso Order Status voice agent workflow

The voice agent template is intended to help customers track the status of their orders. When you call the the phone number linked to the workstream the AI agent is added, the workflow is as follows:

1. The AI agent greets the customer and asks how it can help.
1. The customer says, "I want to track my order."
1. The AI agent recognizes the customer's intent to track an order and prompts the customer to provide the order number or phone number. 
1. The customer can either say the order number or enter it using the dialpad. 
1. The AI agent then validates the order number. If the order number isn't valid, the AI agent prompts the customer to specify the order number again.  
1. The AI agent then asks for your zip code. 
1. The customer can either say the zipcode or type it in. The AI agent validates the zip code. If the zip code isn't valid, the AI agent prompts the customer to specify the zip code again.
1. The AI agent retrieves the status and tells you the status of your order.
1. The AI agent then asks the customer if there's anything else it can help the customer with. Based on the customer's response, the following actions occur:
     - **No**: Call ends. 
     - **Yes**: Call continues, but the AI agent doesn't engage with the call further. In Copilot Studio, you can add an end conversation topic to end the call.
1. The AI agent transfers the customer to a customer service representative based on the workstream configuration, if the customer requests to speak to a service representative at any point during the call. 
