...
description: Learn how to enable intent-based suggestions and next best actions for service representatives in Dynamics 365 Contact Center.
author: gandhamm
ms.author: mgandham
...
Intent-based suggestions and next best actions give customer service representatives (service representatives or representatives) real-time guidance and recommended actions.
...
Before you set up the autonomous agent, make sure the following configurations are in place. Copilot automatically configures supported prerequisites when you enable and save the settings on the **Intent-based suggestions** page:
...
- **Ask a question** is enabled in the Copilot help pane. Learn more in [Configure Copilot features in the help pane](copilot-enable-help-pane.md).
 
- Optionally, configure connectors for the agent to retrieve information from external knowledge sources. Learn more in [Manage custom connectors for AI agents](manage-customer-intent-agent.md#manage-connectors-for-ai-agents-optional).
...
1. In **Enable in experience profiles**, select **Select experience profiles**.
     1. In **Select experience profiles**, select one or more experience profiles.
     1. Select **Save**.
> [!NOTE]
...
When the agent maps the context to a general intent group but not a specific intent, the service representative sees only the high-level intent and a follow-up question to clarify it.
...
- Review a draft response generated for the customer.
- Resolve the case after confirming that the return request is completed.
