...
Create and manage surveys that go out to the customers after a call or conversation ends. When you create a survey in Copilot Service admin center, the application automatically provisions an AI agent (agent) survey to collect customer feedback. Contact centers can improve their quality of service based on the survey responses.
...
- Help supervisors view and review feedback summarized into actionable insights.
...
To publish the survey agent, make sure that the Dataverse connection is established for Copilot Studio. 
...
Publish the survey in Copilot Studio.

1. In Copilot Service admin center, select the survey that you created. The survey opens in Copilot Studio page on a new tab. 
...
Multilingual survey agents require an IVR agent that first detects the customer language. Multilingual survey agents are supported in the voice channel and in-line chat surveys only and aren't supported in link‑mode surveys.
...
Edit your survey agents to fulfill your business needs as follows:
...
Consider the following guidelines when you manage the surveys in Copilot Studio.
...
1. Select the survey and then select **Edit**.
...
1. Select **Powered by Microsoft Copilot Studio**, and then select a survey in the list. Surveys that you create using the customer feedback option and in published state only are displayed.
...
1. On the **Behaviors** tab, scroll to the bottom of the page and enable the **Post-call survey** toggle.
1. In **Customer feedback survey**, select a survey from the list. Surveys that you create using the customer feedback option and in published state only are displayed.
1. Save the changes.
...
> [!NOTE]
> When the post-call survey is on, the system doesn't play the "agent is disconnected" message after the agent disconnects from the call. 
...
1. Go back to Copilot Service admin center, and in **Post case resolution survey**, do the following actions:
